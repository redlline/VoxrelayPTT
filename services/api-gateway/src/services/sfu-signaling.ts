...
export class SfuSignalingHandler {
  ...
  private async handleProduce(
    ...
    const speaker = await floorControlManager.getSpeaker(channelId);
    ...
    if (kind === 'audio' && speaker === this.userId) {
      this.broadcastToChannel(channelId, {
  ...
  private async handleDispatcherForceRelease(channelId: string, targetUserId: string): Promise<void> {
    ...
    const activeSpeaker = await floorControlManager.getSpeaker(channelId);
  ...
  private async handleDispatcherForceReleaseAny(channelId: string): Promise<void> {
    ...
    const speakerId = await floorControlManager.getSpeaker(channelId);
    if (!speakerId) {

...
class UnifiedPlan extends HandlerInterface {
  ...
  Future<HandlerSendResult> send(HandlerSendOptions options) async {
    ...
    if (offer.sdp == null) {
      throw 'send() | createOffer returned null SDP';
    }
    ...
    String? scalabilityMode = (options.encodings.isNotEmpty
            ? options.encodings
            : [RtpEncodingParameters(scalabilityMode: '')])
        .first
        .scalabilityMode;
    ScalabilityMode layers = ScalabilityMode.parse(scalabilityMode ?? '');
    ...
    if (localDesc == null || localDesc.sdp == null) {
      throw 'send() | getLocalDescription returned null';
    }
    localSdpObject = SdpObject.fromMap(parse(localDesc.sdp!));
    offerMediaObject = localSdpObject.media[mediaSectionIdx.idx];
